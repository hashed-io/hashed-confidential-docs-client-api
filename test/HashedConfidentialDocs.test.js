...
describe('HashedConfidentialDocs Integration Tests', () => {
  ...
  test('Should fail for non owner trying to view owned data', async () => {
    ...
    await logout()
    const { authProvider } = await util.getPasswordVaultAuthProvider(2)
    await login(authProvider)
...
async function setupSharedData (id1, id2) {
  const { authProvider: vaultAuthProvider1 } = await util.getPasswordVaultAuthProvider(id1)
  const { authProvider: vaultAuthProvider2 } = await util.getPasswordVaultAuthProvider(id2)
  await login(vaultAuthProvider1)
  const toUserAddress = hcd.address()
  await logout()
  await login(vaultAuthProvider2)
...
async function setupOwnedData (id) {
  const { authProvider: vaultAuthProvider } = await util.getPasswordVaultAuthProvider(id)
